...
public final class JamUi {
    ...
    private static final String COMPANION_PACKAGE="companionPackage",COMPANION_SERVICE=".JamService",COMPANION_ACTIVITY=".MainActivity";
    ...
    private static void ensurePolling(){if(!polling){polling=true;main.post(poll);}}
    ...
    private static final Runnable poll=new Runnable(){public void run(){
        ...
        if(observers.isEmpty()&&!feedEnabled()){polling=false;return;}main.postDelayed(this,sessionActive()?600:3000);
    }};
    private static void bind(Context c){
        if(binding||capability(c).isEmpty())return;Context app=c.getApplicationContext();
        try{connection=new ServiceConnection(){
                public void onServiceConnected(ComponentName n,IBinder b){companion=IJamCompanion.Stub.asInterface(b);ensurePolling();}
                public void onServiceDisconnected(ComponentName n){companion=null;binding=false;connection=null;}
                public void onBindingDied(ComponentName n){companion=null;binding=false;connection=null;try{app.unbindService(this);}catch(Exception ignored){}main.postDelayed(()->bind(app),1500);}
    ...
    static void host(Context c){try{startLayer(c);edit(c,command("HOST"));}catch(Exception e){setup(c);}}    static void pair(Context c){
        ...
        }catch(Exception e){toast(c,"Install the selected Jam Layer package first");}
    }
    private static void setup(Context c){try{c.startActivity(new Intent().setComponent(companionComponent(c,COMPANION_ACTIVITY)));}catch(Exception e){toast(c,"Install the selected Jam Layer package first");}}
    static void join(Context c){
        ...
        TextView hint=new TextView(c);hint.setText("Enter the code shown on the host. Wi-Fi Aware can connect nearby devices even when their LANs differ.");hint.setTextSize(14);hint.setPadding(0,0,0,dp(c,16));content.addView(hint);
        ...
        AlertDialog dialog=new AlertDialog.Builder(c).setTitle("Join with a code").setView(content).setPositiveButton("Join",null).setNeutralButton("Scan QR",(d,w)->{
            try{Activity a=activity(c);if(a!=null)a.startActivityForResult(new Intent("app.morphe.jam.SCAN").setComponent(companionComponent(c,COMPANION_ACTIVITY)).putExtra("cap",capability(c)),18432);}catch(Exception e){setup(c);}
        ...
        if(!code.isEmpty()){
            ...
            AlertDialog popup=new AlertDialog.Builder(c).setTitle("Join your Jam").setMessage("Enter this code on a nearby device. Wi-Fi Aware works across separate LANs when supported. Expires in "+minutes+" minutes.").setView(text)
