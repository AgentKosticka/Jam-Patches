...
public final class JamBar extends LinearLayout {
    ...
    public JamBar(Context c,AttributeSet attrs){
        ...
        LinearLayout labels=new LinearLayout(c);labels.setOrientation(VERTICAL);labels.setGravity(Gravity.CENTER_VERTICAL);labels.setPadding(JamUi.dp(c,12),0,JamUi.dp(c,8),0);addView(labels,new LayoutParams(0,-1,1));
        title=new TextView(c);title.setTextColor(0xffeeeeee);title.setTextSize(14);title.setTypeface(null,android.graphics.Typeface.BOLD);title.setSingleLine();title.setEllipsize(android.text.TextUtils.TruncateAt.END);labels.addView(title);
        subtitle=new TextView(c);subtitle.setTextColor(0xffbbbbbb);subtitle.setTextSize(11);subtitle.setSingleLine();subtitle.setEllipsize(android.text.TextUtils.TruncateAt.END);labels.addView(subtitle);
        action=new TextView(c);action.setTextSize(13);action.setTextColor(0xffeeeeee);action.setGravity(Gravity.CENTER);action.setPadding(JamUi.dp(c,12),0,JamUi.dp(c,12),0);addView(action,new LayoutParams(-2,-1));
        android.util.TypedValue selectable=new android.util.TypedValue();c.getTheme().resolveAttribute(android.R.attr.selectableItemBackground,selectable,true);setBackgroundResource(selectable.resourceId);setOnClickListener(v->JamUi.open(c));setFocusable(true);setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_YES);labels.setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO_HIDE_DESCENDANTS);action.setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO);
        render(JamUi.latest);
    }
